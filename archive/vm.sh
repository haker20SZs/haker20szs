...
if [[ -f "./installed" ]]; then
    ...
    cmds=("mv gotty /usr/bin/" "mv unzip /usr/bin/" "apt-get --fix-broken install -y apt-utils && apt-get clean && apt-get -y update && apt-get -y upgrade && clear" "apt-get -y install sudo curl neofetch python3")
